...
const { rewriteText, getAvailableStyles, isConfigured } = require('./lib/llmService');
...
app.post('/api/upload', upload.single('presentation'), async (req, res) => {
  try {
    ...
    const presentationId = Date.now().toString(36) + Math.random().toString(36).substr(2);
    const slides = await parsePptx(req.file.buffer);
    const html = convertToHtml(slides, presentationId);

    // Store the presentation data for later editing
    presentations.set(presentationId, { slides, originalName: req.file.originalname });
    ...
    res.status(500).json({ error: 'Failed to process presentation: ' + error.message });
  }
});
